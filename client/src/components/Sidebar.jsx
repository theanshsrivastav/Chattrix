...
const Sidebar = () => {
    ...
    return (
        <div className={`bg-[#8185B2]/75 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
            <div className='pb-5'>
                <div className='flex justify-between items-center'>
                    <h1 className='text-white text-2xl font-extrabold tracking-wide'>Chattrix</h1>
                    <div className='relative py-2 group'>
                        <img src={assets.menu_icon} alt="menu_icon" className='max-h-5 cursor-pointer border-2 border-gray-900 rounded bg-gray-900'/>
                    ...
                    <div key={index} onClick={()=>{setSelectedUser(user); setUnseenMessages(prev => ({...prev, [user._id]: 0}))}} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                        ...
                        <div className='flex flex-col leading-5'>
                            <p>{user.fullName}</p>
                            {
                                ...
                                : <span className='text-teal-100 text-xs'>Offline</span>
                            }
                        </div>
